import { YuclaseClient } from '../src/index';
...
async function basicExample() {
  ...
  try {
    ...
    // Test ping
    const pong = await client.ping('Hello from TypeScript!');
    console.log('Ping response:', pong);

    // Create a topic
    const topicName = 'typescript-demo';
    console.log(`Creating topic: ${topicName}`);
    await client.createTopic(topicName);

    // Enqueue some messages
    console.log('Enqueuing messages...');
    const offset1 = await client.enqueue(topicName, 'Hello from TypeScript client!');
